...
---
`r chunk_reveal("releveling", break_type = "rotate", float = "none", widths = c(5, 1), title = "# Setting factor levels")`

```{r releveling, include = FALSE}
ds <- ds %>% select(name, eye_f)
ds #ROTATE
ds %>% mutate(eye_f = factor(eye_f, levels = c("blue", "brown"))) #ROTATE
ds %>% mutate(eye_f = factor(eye_f, levels = c("blue", "brown", "yellow"),
                             labels = c("bl","br","yl"))) #ROTATE
ds %>% mutate(eye_f = fct_lump(eye_f, n = 3)) #ROTATE
