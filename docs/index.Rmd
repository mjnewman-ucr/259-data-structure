...
---
`r chunk_reveal("pivotlong", break_type = "rotate", float = "none", widths = c(8, 1))`
...
```{r}
ds <- read_delim(list.files(path = "data_headers", full.names = T),
      col_names = c("field", "value"), col_types = c("cc"), delim = " ", n_max = 7,
      id = "filename")
ds <- ds %>% pivot_wider(id_cols = "filename", names_from = "field", values_from = "value")
ds
```
---
...
- Data frames can be joined in three main ways
  ...
  * Matching joins, where columns are added from one data frame to another based on matching key information
