...
str_extract_all(ds$word, "(?<=[:space:])[:alpha:]+", simplify = T) #ROTATE
...
```{r combiningstrings, include = T}
age <- seq(from = 12, to = 24, by = .5) 
paste0("data_raw/vocab", age, ".csv") #ROTATE
str_glue("data_raw/vocab{age}.csv") #ROTATE
str_glue("Today's date is {Sys.Date()} and the time zone is {Sys.timezone()}") #ROTATE
